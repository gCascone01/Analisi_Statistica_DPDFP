...
qnorm = qnorm (1 - alpha / 2, mean =0 , sd =1)
qnorm
...
D_Francia <- as.vector(D_dataset["Francia",])
n_Francia = length(D_Francia)

mean_Francia <- mean(D_Francia)

sd_Francia <- sd(D_Francia)

a <- numeric(4)

for(i in 1:4){
  a[i] <- qnorm(0.2*i, mean=mean_Francia, sd=sd_Francia)
}

r <- 5

nint <- numeric(r)
nint [1] <- length(which(D_Francia < a [1]) )
for(i in 2:4) {
  nint[i] <- length(which((D_Francia >= a[i-1]) & D_Francia < a[i]))
}
nint[5] <- length ( which ( D_Francia >= a [4]) )

chi2 <- sum ((( nint - n_Francia* 0.2) / sqrt ( n_Francia* 0.2) ) ^2)
chi2
